...
- Encryption and decryption of wrap messages: `wrap::Message::decrypt` and
  `wrap::Plaintext::encrypt` ([#494])
- Helpers for reading RSA public keys ([#509])
- Import of wrapped RSA keys ([#511])
- DER-encoded ECDSA signatures ([#506])
- RSA signers for PSS and PKCS#1 v1.5 ([#493])
- Wrapping support for ECDSA keys: `wrap::Plaintext::ecdsa` and
  `wrap::Plaintext::from_ecdsa` ([#549])
- ECDSA support for NIST P-521 (`asymmetric::Algorithm::EcP521`) ([#574])
- `signature::hazmat::PrehashSigner` implementation for RSA-PKCS ([#577])
- `Client::export_wrapped_with_seed`, for exporting Ed25519 keys with their
  seed ([#661])
- `Client::change_authentication_key`, wrapping the `ChangeAuthenticationKey`
  command (`0x6c`, firmware 2.2.0+). Replaces an authentication key's key
  material while preserving its metadata. Only the key backing the current
  session may be changed ([#664])
- `session::MAX_COMMANDS_PER_SESSION` is now public, so callers who have
  disabled session re-establishment can monitor how close a session is to the
  command limit ([#625])
- `client::ErrorKind::AuthenticationKeyError` and
  `session::ErrorKind::AuthenticationKeyError`, distinguishing "authentication
  key does not exist" from "authentication failed" ([#585])
- `std::fmt::Debug` implementations for `Client` and `Connector`. Credentials
  are deliberately excluded from the output ([#660])
- Deserializers can now be reused with self-describing formats such as JSON
  ([#659])
- `Session::close`, for callers who want to observe errors when releasing a
  session rather than relying on the `Drop` handler. Reachable through
...
### Changed
- **MSRV is now 1.88** (was 1.67) ([#551], [#568], [#571], [#613], [#616], [#680])
- **Breaking:** the `Connectable` trait now requires `std::fmt::Debug`.
  Downstream crates implementing custom connectors must add it ([#660])
- **Breaking:** `client::ErrorKind` and `session::ErrorKind` gain new variants.
  Neither enum is `#[non_exhaustive]`, so exhaustive matches must be updated
  ([#585])
- **Breaking:** major dependency upgrades across the RustCrypto stack:
  - `ecdsa` v0.16 -> v0.17 ([#610], [#671])
  - `signature` v2 -> v3 ([#614])
  - `ed25519` v2 -> v3, `ed25519-dalek` v2 -> v3
  - `p256`/`p384` v0.13 -> v0.14, `k256` v0.13 -> v0.14, plus new `p521`
  - `rsa` v0.9 -> v0.10
  - `x509-cert` v0.2 -> v0.3
  - `aes` v0.8 -> v0.9, `cbc` v0.1 -> v0.2, `ccm` v0.5 -> v0.6,
    `cmac` v0.7 -> v0.8, `hmac` v0.12 -> v0.13, `pbkdf2` v0.12 -> v0.13
  - `digest` v0.10 -> v0.11, `sha2` v0.10 -> v0.11, plus new `sha1`
  - `rand_core` v0.6 -> v0.10, plus new `rand` v0.10
  - `thiserror` v1 -> v2 ([#593])
- Dependency requirements now name stable releases rather than pre-releases,
  except `rsa` (0.10) and `ccm` (0.6), which have not yet shipped one ([#679])
- **Breaking:** the `passwords` and `untested` features no longer imply `sha2`,
  which is now a mandatory dependency
- **Breaking:** removed `From<&str> for object::Label`, which panicked on
  labels longer than 40 bytes. Use `parse()` and handle the error ([#694])
- **Breaking:** `Client::change_authentication_key` takes `&mut self`, so that
  cached credentials are updated when a client rotates its own session key
  ([#664])
- `Capability`, `Domain` and the algorithm types now implement `PartialOrd` and
  `Ord`. Flag ordering is by bit value and is *not* set inclusion; `Algorithm`
  and `rsa::Algorithm` order by wire tag rather than declaration order ([#695])
- `object::Filter` derives `Clone`, `Debug`, `Serialize` and `Deserialize`. Its
  HSM wire codec is now named `to_wire`/`from_wire` to keep the two encodings
  apart ([#694])
- ECDSA signers are now generated by a macro rather than written out per curve
  ([#569])
- `rusb` API break mitigated ([#552])
- Documentation improvements for the `client` module ([#611]) and MockHSM
  ([#608], [#609])

### Fixed
- Audit log entries were silently truncated, and the wrong object length was
  reported for wrapped RSA keys ([#559], [#653])
- ECDSA public key deserialization for ECC curves ([#558]) and ECDSA
  serialization when wrapping ([#658])
- `export_wrapped` only emits the `include_seed` attribute when it is
  requested; serializing it as zero made the device reply `WrongLength`
  ([#673])
- RSA-PSS now checks message size after hashing rather than before ([#603])
- MockHSM appends the modulus to the message for wrapped RSA keys, matching
  device behavior ([#566])
- Permission errors are correctly propagated from `Client::reset_device`
  ([#620]), and MockHSM checks capabilities before resetting ([#622])
- Message serialization failures are handled rather than panicking ([#623])
- Sessions no longer time out unnecessarily ([#555])
- `Deserialize` implementation for OAEP ([#545]) and forced array serialization
  in the serializer ([#546])
- Capability names in error messages ([#662])
- The HTTP connector's `status` endpoint now separates keys from values with
  `=` rather than a newline, so `yubihsm-shell` can connect to it ([#675])
- p384 support ([#548])
- Sessions are released to the HSM when dropped again, rather than being held
  until they time out. The device permits a limited number of concurrent
  sessions, so leaking them exhausted the pool. The destructor no longer runs
  transport I/O while unwinding, which could abort the process on a poisoned
  connector mutex ([#692], [#495])
...
- Took patched releases of three dependencies with open advisories ([#680]):
  - `cmov` 0.5.4 — [GHSA-3rjw-m598-pq24], wrong results from `Cmov`/`CmovEq` on
    aarch64 when the high bits of registers are set. Reached this crate through
    `crypto-bigint` -> `crypto-primes` -> `rsa`, i.e. the RSA constant-time
    arithmetic path.
  - `time` 0.3.55 — [GHSA-r6v5-fh4h-64xc], stack exhaustion denial of service.
  - `rand` 0.10.2 — [GHSA-cq8v-f236-94qc], unsoundness with a custom logger
    using `rand::rng()`.

- Building `mockhsm` without debug assertions now requires the explicit
  `mockhsm-in-release` feature. MockHSM accepts any credentials and keeps key
  material in process memory; cargo features are additive, so `mockhsm` alone
  could be switched on from elsewhere in a dependency graph ([#693])
...
[#460]: https://github.com/iqlusioninc/yubihsm.rs/pull/460
[#493]: https://github.com/iqlusioninc/yubihsm.rs/pull/493
[#494]: https://github.com/iqlusioninc/yubihsm.rs/pull/494
[#495]: https://github.com/iqlusioninc/yubihsm.rs/issues/495
[#506]: https://github.com/iqlusioninc/yubihsm.rs/pull/506
[#509]: https://github.com/iqlusioninc/yubihsm.rs/pull/509
[#510]: https://github.com/iqlusioninc/yubihsm.rs/pull/510
[#511]: https://github.com/iqlusioninc/yubihsm.rs/pull/511
[#545]: https://github.com/iqlusioninc/yubihsm.rs/pull/545
[#546]: https://github.com/iqlusioninc/yubihsm.rs/pull/546
[#548]: https://github.com/iqlusioninc/yubihsm.rs/pull/548
[#549]: https://github.com/iqlusioninc/yubihsm.rs/pull/549
[#551]: https://github.com/iqlusioninc/yubihsm.rs/pull/551
[#552]: https://github.com/iqlusioninc/yubihsm.rs/pull/552
[#553]: https://github.com/iqlusioninc/yubihsm.rs/pull/553
[#555]: https://github.com/iqlusioninc/yubihsm.rs/pull/555
[#558]: https://github.com/iqlusioninc/yubihsm.rs/pull/558
[#559]: https://github.com/iqlusioninc/yubihsm.rs/pull/559
[#566]: https://github.com/iqlusioninc/yubihsm.rs/pull/566
[#568]: https://github.com/iqlusioninc/yubihsm.rs/pull/568
[#569]: https://github.com/iqlusioninc/yubihsm.rs/pull/569
[#571]: https://github.com/iqlusioninc/yubihsm.rs/pull/571
[#574]: https://github.com/iqlusioninc/yubihsm.rs/pull/574
[#576]: https://github.com/iqlusioninc/yubihsm.rs/pull/576
[#577]: https://github.com/iqlusioninc/yubihsm.rs/pull/577
[#585]: https://github.com/iqlusioninc/yubihsm.rs/pull/585
[#593]: https://github.com/iqlusioninc/yubihsm.rs/pull/593
[#603]: https://github.com/iqlusioninc/yubihsm.rs/pull/603
[#608]: https://github.com/iqlusioninc/yubihsm.rs/pull/608
[#609]: https://github.com/iqlusioninc/yubihsm.rs/pull/609
[#610]: https://github.com/iqlusioninc/yubihsm.rs/pull/610
[#611]: https://github.com/iqlusioninc/yubihsm.rs/pull/611
[#613]: https://github.com/iqlusioninc/yubihsm.rs/pull/613
[#614]: https://github.com/iqlusioninc/yubihsm.rs/pull/614
[#616]: https://github.com/iqlusioninc/yubihsm.rs/pull/616
[#620]: https://github.com/iqlusioninc/yubihsm.rs/pull/620
[#622]: https://github.com/iqlusioninc/yubihsm.rs/pull/622
[#623]: https://github.com/iqlusioninc/yubihsm.rs/pull/623
[#625]: https://github.com/iqlusioninc/yubihsm.rs/pull/625
[#653]: https://github.com/iqlusioninc/yubihsm.rs/pull/653
[#658]: https://github.com/iqlusioninc/yubihsm.rs/pull/658
[#659]: https://github.com/iqlusioninc/yubihsm.rs/pull/659
[#660]: https://github.com/iqlusioninc/yubihsm.rs/pull/660
...
[#693]: https://github.com/iqlusioninc/yubihsm.rs/pull/693
