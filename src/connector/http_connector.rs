...
impl HttpConnector {
    ...
    fn post(&self, path: &str, uuid: Uuid, mut body: Vec<u8>) -> Result<Vec<u8>, Error> {
...
impl ResponseReader {
    ...
    pub fn read(socket: &mut TcpStream) -> Result<Self, Error> {
        let mut buffer = Self {
    ...
    fn parse_headers(&mut self) -> Result<(), Error> {
        ...
        match header_iter.next() {
            ...
            None => fail!(
                ConnectorError::ResponseError,
                "HTTP response status line missing!"
            ),
        }
